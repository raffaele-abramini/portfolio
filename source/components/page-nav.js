...
class PageNav extends PureComponent {
  ...
  toggleNavButton = createRef()

  closeNavButton = createRef()
    ...
    return (
      <section className={navClasses}>
        ...
        <aside className="c-page-nav__aside">
          <button className="c-page-nav__toggle-inner c-button--no-style c-button--no-style--inverted"
                  aria-label="Close side navigation"
                  onClick={() => this.handleToggleClick()}
                  ref={this.closeNavButton}>
  ...
  handleToggleClick() {
    if (this.props.isNavOpen) {
      this.toggleNavButton.current.focus()
    } else {
      this.closeNavButton.current.focus()
    }
